...
#' Prepare Stock Synthesis data for recdevplot()
#'
...
#' recdevplot(data)
recdevplot_prep_SS <- function(ss_mle,
                               ss_mcmc = NULL,
                               scenarios = NULL,
  ...
  if (!MCMC) {
    ...
    for (scenario in scenarios) {
      ...
      if (is.null(ss_mle[[scenario]]$recruitpars)) {next}

      tmp <- ss_mle[[scenario]]$recruitpars |>
        dplyr::filter(type== "Main_RecrDev") |>
        dplyr::mutate(upper=Value + 1.96 * Parm_StDev,
                      lower=Value - 1.96 * Parm_StDev) |>
        dplyr::mutate(scenario = scenario,
                      ...
                      ub = upper,
                      lb= lower) |>
        dplyr::select(year, value, ub, lb, scenario, method)
