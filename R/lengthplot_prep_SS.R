...
#' Prepare Stock Synthesis data for length plot
#'
...
#'
#' @return A data frame with variables year (int), fleet (int), bin (int), sex (chr), obs (num), exp (num), scenario (int)
#' @export
#'
#' @examples
#' data <- lengthplot_prep_SS(ss_mle)
#' lengthplot(data, show_fits=FALSE)
lengthplot_prep_SS <- function(ss_mle,
                               scenarios = NULL) {
  ...
  for (scenario in scenarios) {
    ...
    tmp <- ss_mle[[scenario]]$lendbase |>
      dplyr::select(year=Yr, fleet=Fleet, bin=Bin, sex, obs=Obs, exp=Exp, partition = Part) |>
      dplyr::mutate(obs = ifelse(sex==2,obs*-1,obs),
                    exp = ifelse(sex==2,exp*-1,exp)) |>
      dplyr::mutate(sex = dplyr::recode(as.character(sex),
                                        "1" = "Female",
                                        "2" = "Male",
                                        "3" = "Sexes combined",
                                        "0" = "Sexes combined")) |>
