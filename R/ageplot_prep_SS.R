...
#'
#' @return A data frame with variables year (int), fleet (int), bin (int), sex (chr), obs (num), exp (num), scenario (num)
#' @export
#'
#' @examples
#' \dontrun{
...
#' ageplot(data)
#' }
ageplot_prep_SS <- function(ss_mle,
                            scenarios = NULL) {
  ...
  for (scenario in scenarios) {
    ...
    tmp <- ss_mle[[scenario]]$agedbase |>
      dplyr::select(year=Yr, fleet=Fleet, bin=Bin, sex, obs=Obs, exp=Exp) |>
