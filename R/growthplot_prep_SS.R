...
#'   growthplot(data)
#' }
growthplot_prep_SS <- function(ss_mle, ss_mcmc = NULL, scenarios = NULL, season = 1) {
  ...
  if(MCMC) {
    ...
    for(scenario in scenarios) {
      ...
      strings_to_include <- c(
        "L_at_Amin_Fem_GP_1", "L_at_Amax_Fem_GP_1", "VonBert_K_Fem_GP_1", "CV_young_Fem_GP_1", "CV_old_Fem_GP_1", "SD_young_Fem_GP_1", "SD_old_Fem_GP_1",
        "L_at_Amin_Mal_GP_1", "L_at_Amax_Mal_GP_1", "VonBert_K_Mal_GP_1", "CV_young_Mal_GP_1", "CV_old_Mal_GP_1", "SD_young_Mal_GP_1", "SD_old_Mal_GP_1"
      )
      ...
      if(nrow(fixed_parameters) > 0) {
        ss_mcmc_growth_fixed <- fixed_parameters |>
          dplyr::select(Label, Value) |>
          tidyr::pivot_wider(names_from = Label, values_from = Value) |>
          tidyr::uncount(nrow(ss_mcmc[[scenario]]))
        ss_mcmc_growth <- cbind(ss_mcmc_growth, ss_mcmc_growth_fixed)
      }
      ...
      if(nsexes == 2) {
        ...
        data <- dplyr::bind_rows(data, sigma_fun(
          data_mal,
          sigma_var_mal,
          A1 = ss_mle[[scenario]]$Growth_Parameters$A1[2],  # From control file
          A2 = ss_mle[[scenario]]$Growth_Parameters$A2[2]   # From control file
