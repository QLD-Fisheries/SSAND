...
data <- format_logbooks(raw_logbook_data) # format logbooks for investigation plots
investigate(data) # produce an Rmd and HTML to view logbook data
```

### Biological data investigation
```{r include=FALSE, eval=FALSE}
investigate_biology(data) # produce an Rmd and HTML to view biological data
```

### Upset plot
```{r echo=TRUE, include=TRUE}
head(upset) # View sample data
upsetplot(upset, species = "Glitterfin snapper")
```
...
![An example likelihood profile plot, produced using likelihoodprofileplot()](fig/likelihood_example.png)
...
![An example management plot, produced using managementplot()](fig/management_example.png)
